...
{   //此函数会临时占用T4和T11定时器进行计算，使用完成后会进行释放
    ...
    _T4T3M = T4T3M; _IE2 = IE2; _IE = IE; EA = 0;
...
void timer_value_setting(timer_num num, float time) // 单位秒（s）
{
    char _1tx12t = 0; // 默认1T
    ...
    switch (num)
    {
    case Timer0:INIT_TIMER(TM0,TH0,TL0);TIMER_TxTR(0,TR0);break; // 开启中断
    case Timer1:INIT_TIMER(TM1,TH1,TL1);TIMER_TxTR(1,TR1);break; // 开启中断
    case Timer2:INIT_TIMER(TM2,T2H,T2L);TIMER_TxTR(2,T2R);break; // 开启中断
    case Timer3:INIT_TIMER(TM3,T3H,T3L);TIMER_TxTR(3,T3R);break; // 开启中断
    case Timer4:INIT_TIMER(TM4,T4H,T4L);TIMER_TxTR(4,T4R);break; // 开启中断
    case Timer11:INIT_TIMER(T11,T11H,T11L);SET_REGISTER_BIT(T11CR, _1tx12t == 0 ? 0 : 1, 4); // 打开1T/12T
        SET_REGISTER_BIT(T11CR, 1, 7);break; //启动定时器11.
    default:break;
    }
}
...
void set_timer_mode(timer_num num, ...)
{
    ...
    timer_value_setting(num, set_timer_value); // 设置定时器值
