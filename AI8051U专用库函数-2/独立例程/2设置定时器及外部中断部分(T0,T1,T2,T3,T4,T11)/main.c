...
void main(void)
{
    ...
    // 设置T1=P35为准双向模式，手动使用变换输出，来使得T1在计数模式下计数
    ...
    // 设置后会自动启动定时器和对应中断，只需要使用get_timer_state查询即可
		...
		//设置Timer1为计数模式，计数输入端口固定为P35（具体详见引脚图和技术手册说明）
		set_timer_mode(Timer2, "10ms", Timer_End);
		// 设置定时10ms，来主动触发P35端口取反，测试T1的脉冲计数功能
    EA = 1; // 打开总中断
    while (1)
    {
            ...
            P02 = ~P02; // 对应IO口上的LED灯1S变化一次
				if (get_timer_state(Timer2))
				{
					...
					if(cnt == 50)
					{
						...
						//正常应该为cnt:50, timer_cnt:25，因为取反中， 两次才是完整的一个周期变化
						cnt = 0;
					}
				}
    }
