...
void SPI_config(void)
{
    ...
    SPI_InitStructure.TimeOutEnable  = ENABLE;          //从机超时使能, ENABLE,DISABLE
    SPI_InitStructure.TimeOutINTEnable  = ENABLE;       //超时中断使能, ENABLE,DISABLE
    SPI_InitStructure.TimeOutScale  = TO_SCALE_1US;     //超时时钟源选择, TO_SCALE_1US,TO_SCALE_SYSCLK
    SPI_InitStructure.TimeOutTimer  = 1000ul;           //超时时间, 1 ~ 0xffffff
...
void main(void)
{
    ...
    while (1)
    {
        ...
        if(SPI_RxTimerOut)
        {
            ...
            {
                for(i=0; i<SPI_RxCnt; i++)    TX1_write2buff(SPI_RxBuffer[i]);    //通过串口输出数据
            }
