...
Lin从机模式收发测试：
收到一个非本机应答的完整帧后通过串口2(P4.2,P4.3)输出.
收到一个本机应答的帧头后(例如：ID=0x12), 发送缓存数据进行应答.
需要修改头文件 "AI8051U_UART.h" 里的定义 "#define	PRINTF_SELECT  UART2"，通过串口2打印"printf"信息
...
//	COMx_InitStructure.UART_BRT_Use   = BRT_Timer2;		//选择波特率发生器, BRT_Timer2 (注意: 串口2固定使用BRT_Timer2, 所以不用选择)
	...
	P1_MODE_IO_PU(GPIO_Pin_6 | GPIO_Pin_7); //P1.6,P1.7 设置为准双向口
