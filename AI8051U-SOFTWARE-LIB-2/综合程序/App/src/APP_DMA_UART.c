...
u8 xdata DmaBuffer[512];    //收发共用缓存，同时使用多路串口时每个串口需分别定义缓存，以免相互干扰
...
void DMA_UART_init(void)
{
    ...
    //----------------------------------------------
    DMA_UART_InitStructure.DMA_TX_Length = 511;             //DMA传输总字节数 (0~65535) + 1
    DMA_UART_InitStructure.DMA_TX_Buffer = (u16)DmaBuffer;  //发送数据存储地址
    DMA_UART_InitStructure.DMA_RX_Length = 511;             //DMA传输总字节数 (0~65535) + 1
...
void Sample_DMA_UART(void)
{
    ...
    if(COM1.RX_TimeOut)       //接收一串数据结束，产生超时中断
    {
        ...
        number = ((u16)DMA_UR1R_DONEH << 8) + DMA_UR1R_DONE; //获取已接收字节个数
        CLR_TI(); //清除发送标志（DMA发送完不会自动清除标志位）
        printf("number=%u\r\n",number);

        SET_DMA_UR1R_CR(DMA_DISABLE);
        DMA_UR1T_AMT = (u8)(number-1);       //设置传输总字节数(低8位)：n+1
        DMA_UR1T_AMTH = (u8)((number-1)>>8); //设置传输总字节数(高8位)：n+1
    ...
    if(COM2.RX_TimeOut)       //接收一串数据结束，产生超时中断
    {
        ...
        number = ((u16)DMA_UR2R_DONEH << 8) + DMA_UR2R_DONE; //获取已接收字节个数
        CLR_TI2(); //清除发送标志（DMA发送完不会自动清除标志位）

        SET_DMA_UR2R_CR(DMA_DISABLE);
        DMA_UR2T_AMT = (u8)(number-1);       //设置传输总字节数(低8位)：n+1
        DMA_UR2T_AMTH = (u8)((number-1)>>8); //设置传输总字节数(高8位)：n+1
    ...
    if(COM3.RX_TimeOut)       //接收一串数据结束，产生超时中断
    {
        ...
        number = ((u16)DMA_UR3R_DONEH << 8) + DMA_UR3R_DONE; //获取已接收字节个数
        CLR_TI3(); //清除发送标志（DMA发送完不会自动清除标志位）

        SET_DMA_UR3R_CR(DMA_DISABLE);
        DMA_UR3T_AMT = (u8)(number-1);       //设置传输总字节数(低8位)：n+1
        DMA_UR3T_AMTH = (u8)((number-1)>>8); //设置传输总字节数(高8位)：n+1
    ...
    if(COM4.RX_TimeOut)       //接收一串数据结束，产生超时中断
    {
        ...
        number = ((u16)DMA_UR4R_DONEH << 8) + DMA_UR4R_DONE; //获取已接收字节个数
        CLR_TI4(); //清除发送标志（DMA发送完不会自动清除标志位）

        SET_DMA_UR4R_CR(DMA_DISABLE);
        DMA_UR4T_AMT = (u8)(number-1);       //设置传输总字节数(低8位)：n+1
        DMA_UR4T_AMTH = (u8)((number-1)>>8); //设置传输总字节数(高8位)：n+1
