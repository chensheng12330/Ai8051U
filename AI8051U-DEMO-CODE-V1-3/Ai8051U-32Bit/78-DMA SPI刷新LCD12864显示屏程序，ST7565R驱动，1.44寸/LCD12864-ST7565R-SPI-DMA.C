...
//脚位定义根据自身所买的屏的脚位顺序进行定义（以上是作者测试屏的脚位定义）：


//sbit P_LCD_CS   = P3^5;	// 1----CS		使能信号, L有效
//sbit P_LCD_RST  = P4^7;	// 2----RST		复位信号, L复位, H正常工作
//sbit P_LCD_RS   = P1^1;	// 3---RS(或A0)	数据/指令选择, H选择数据, L选择指令
//sbit P_LCD_SDA  = P3^3;	// 4----SDA		串行数据			9----ROM_CS			注意UC1701的对比度调整
//sbit P_LCD_SCLK = P3^2;	// 5----SCK		串行时钟			10---ROM_SCK
//脚位定义根据自身所买的屏的脚位顺序进行定义（以上是实验箱原理图的脚位定义）：
